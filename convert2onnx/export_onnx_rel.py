...
def make_parser():
    ...
    parser.add_argument(
        "--output-name-tpsmm", type=str, default="tpsmm_rel.onnx", help="output name of tpsmm models"
    )
    parser.add_argument(
        "--input-kp", default="source", type=str, help="input node name of kp onnx model"
    )
    parser.add_argument(
        "--input-tpsmm", default=["kp_source", "source", "kp_driving", "driving"], type=list,
...
def main():
    ...
    # added dummy_source_kp for relative mode
    # also need to change modules/tpsmm.py
    dummy_source_kp = torch.randn(args.batch_size, 50, 2)
    ...
    torch.onnx.export(
        tpsmm_model,
        (dummy_driving_kp, dummy_source, dummy_source_kp, dummy_driving),
